// src/components/geo-snapshot-table.jsx
...
export function GeoSnapshotTable() {
  ...
  const generatedData = useMemo(() => {
    ...
    const salesMap = new Map(
      (rawData.salesByState || []).map(s => [s.state, s])
    );
    const callsMap = new Map(
      (rawData.callsByState || []).map(c => [c.state, c])
    );
    ...
    return Array.from(allStates).map(state => {
      const sales = salesMap.get(state);
      const calls = callsMap.get(state);
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => navigate(`/geo-profile/${geo.stateRaw}`)}
      ...
      <div>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>GEO Snapshot</CardTitle>
              <CardDescription>Overview of sales performance by state</CardDescription>
            </div>
            <div className="flex items-center py-4">
              <Input
                placeholder="Search by state name..."
                value={table.getColumn("geoid")?.getFilterValue() ?? ""}
                onChange={(event) =>
                  table.getColumn("geoid")?.setFilterValue(event.target.value)
                }
                className="max-w-sm"
              />
            </div>
